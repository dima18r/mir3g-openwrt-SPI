...
```
Процессор пытается инициализировать PCIe, делает две попытки, но зависает, поскольку ответа не получает. Мной было принято решение отключить этот модуль, либо вообще возможность опроса.
